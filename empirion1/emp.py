...
def typeB(items):
    ...
    with namespace:
        ...
        if representationInstance:                                                         # если такой экземпляр найден, то:
            ...
            representationInstance.hasMeasurement_Value.append(instUnit) 
    return True
...
onto_path.append(r"C:\Users\User\Downloads")      # указываю путь к локальной папке, в которой хранится онтология
...
empirion_namespace = empirion.get_namespace("http://www.tempuri.org/empirion#")
...
for f_name in os.listdir(work_directory):                   # пробегаю по всем файлам этой папки
    if f_name.endswith('_toonto.txt'):                      # рассматриваю только файлы, имена которых заканчиваются на _toonto.txt
        ...
        with open(f_name,'r') as file:                      # открываю файл
            ...
            print(dataFileStr)
            dataFileClass = empirion.DataFile               # получаю класс DataFile из онтологии empirion
            with namespace:
            ...
            print(varDescrStr)
            varDescrClass = empirion.VariableDescriptionFile    # получаю класс VariableDescriptionFile из онтологии empirion
            print(varDescrClass)
            with namespace:
